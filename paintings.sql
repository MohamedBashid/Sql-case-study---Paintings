...
select count(*) from product_size ps where ps.sale_price > ps.regular_price;

-- 4. Identify the paintings whose asking price is less than 50% of its regular price
select * from product_size ps where ps.sale_price < (ps.regular_price/2);

--5. Which canva size costs the most?
select cs.label,ps.sale_price from canvas_size cs join product_size ps on cs.size_id::text = ps.size_id
where cs.size_id::text = (select ps.size_id from product_size ps order by sale_price desc limit 1) order by ps.sale_price desc limit 1;
...
select m.name from museum m where m.city similar to '[0-9]%';
...
count(*) over(partition by s.subject) as famous
from subject s order by famous desc limit 10;
...
select m.name as name, m.state as state, x.op as oopen, x.day as day from
(select *, 
to_timestamp(open,'HH:MI AM') as op, 
to_timestamp(close,'HH:MI PM') as cl, 
to_timestamp(close,'HH:MI PM') - to_timestamp(open,'HH:MI AM') as duration
from museum_hours mh order by duration desc limit 1) x
